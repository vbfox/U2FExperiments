...
namespace BlackFox.U2F.Server.impl
{
	...
	{
		private static readonly DerObjectIdentifier transportExtensionOid = new DerObjectIdentifier("1.3.6.1.4.1.45724.2.1.1");
		...
		public Iu2FServerReferenceImpl(IChallengeGenerator challengeGenerator
			, IDataStore dataStore, IServerCrypto cryto, 
		...
		public SecurityKeyData ProcessRegistrationResponse(RegistrationResponse registrationResponse, long currentTimeInMillis)
		{
			...
			var signedBytes = RawMessageCodec.EncodeRegistrationSignedBytes(appIdSha256, browserDataSha256, keyHandle, userPublicKey);
			...
			var securityKeyData = new SecurityKeyData(currentTimeInMillis, transports, keyHandle, userPublicKey, attestationCertificate, 0);
		...
		public SecurityKeyData ProcessSignResponse(SignResponse signResponse)
		{
            ...
            var appIdSha256 = cryto.ComputeSha256(Encoding.UTF8.GetBytes(appId));
			var browserDataSha256 = cryto.ComputeSha256(Encoding.UTF8.GetBytes(browserData));
			var signedBytes = RawMessageCodec.EncodeAuthenticateSignedBytes(appIdSha256, userPresence, counter, browserDataSha256);
			log.Info("Verifying signature of bytes " + signedBytes.ToHexString());
